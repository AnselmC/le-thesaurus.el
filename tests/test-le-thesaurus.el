...
(describe "le-thesaurus correctly extracts synonyms from parsed JSON response."
  ...
  (it "Returns the synonyms on simple payload"
    (let ((payload '((data . ((definitionData . ((definitions . [((synonyms . [((term . "hello")) ((term . "world"))]))]))))))))
      (expect (le-thesaurus--parse-synonyms-in-response payload) :to-equal '["hello" "world"])))
  (it "Returns the synonyms from full example json payload"
    (let ((payload (json-read-file "tests/data/test-response.json")))
      (expect (le-thesaurus--parse-synonyms-in-response payload) :to-equal '["reference book" "glossary" "lexicon" "onomasticon" "terminology" "vocabulary" "language reference book" "sourcebook" "storehouse of words" "treasury of words" "word list"]))))
...
(describe "le-thesaurus correctly works with thesaurus.com."
  (it "Returns the expected list of synonyms for 'thesaurus'"
    ...
    (let ((word "thesaurus")
      ...
      (expect (le-thesaurus--ask-thesaurus-for-synonyms word) :to-equal expected-synonyms)))
